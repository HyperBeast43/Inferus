...
namespace Content.Client.Lobby.UI
{
    ...
    public sealed partial class HumanoidProfileEditor : BoxContainer
    {
        ...
        /// <summary>Inferus – job title OptionButtons, closed when the jobs list scrolls.</summary>
        private readonly List<OptionButton> _jobTitleButtons = new();
        private bool _jobTitleScrollWired;
        ...
        /// <summary>
        /// Inferus – close any open job-title OptionButton popups when the jobs ScrollContainer moves,
        /// so dropdown items do not float detached over the list.
        /// </summary>
        private void EnsureJobTitleScrollClose()
        {
            if (_jobTitleScrollWired)
                return;

            // JobList lives inside a ScrollContainer in the XAML.
            Control? parent = JobList.Parent;
            while (parent != null && parent is not ScrollContainer)
                parent = parent.Parent;

            if (parent is not ScrollContainer scroll)
                return;

            scroll.OnScrolled += () => CloseJobTitleDropdowns();
            _jobTitleScrollWired = true;
        }

        private void CloseJobTitleDropdowns()
        {
            foreach (var btn in _jobTitleButtons)
            {
                // OptionButton keeps its Popup as the parent of public OptionsScroll.
                if (btn.OptionsScroll.Parent is Popup popup)
                    popup.Close();
            }
        }
        ...
        public void RefreshJobs()
        {
            ...
            _jobTitleButtons.Clear();
            EnsureJobTitleScrollClose();
            ...
            foreach (var department in departments)
            {
                ...
                foreach (var job in jobs)
                {
                    ...
                    // Inferus – alternate job title selector (beside Loadout)
                    ...
                    var titleOptions = JobTitleHelpers.GetAllTitleOptions(job);
                    var hasAlternates = JobTitleHelpers.HasAlternates(job);

                    if (!hasAlternates)
                    {
                        titleButton.AddItem(job.LocalizedName, 0);
                        titleButton.Select(0);
                        titleButton.Disabled = true;
                        titleButton.ToolTip = Loc.GetString("humanoid-profile-editor-job-title-no-alternates");
                    }
                    else
                    {
                        for (var i = 0; i < titleOptions.Count; i++)
                        {
                            var locId = titleOptions[i];
                            titleButton.AddItem(Loc.GetString(locId), i);
                            titleButton.SetItemMetadata(i, (string)locId);
                        }

                        var current = Profile?.JobTitles.GetValueOrDefault(job.ID) ?? (string)job.Name;
                        var selectedIdx = 0;
                        for (var i = 0; i < titleOptions.Count; i++)
                        {
                            if ((string)titleOptions[i] == current)
                            {
                                selectedIdx = i;
                                break;
                            }
                        }
                        titleButton.Select(selectedIdx);

                        titleButton.OnItemSelected += args =>
                        {
                            titleButton.SelectId(args.Id);
                            var meta = titleButton.GetItemMetadata(args.Id);
                            var locId = meta as string ?? (string)titleOptions[args.Id];
                            if (locId == (string)job.Name)
                                Profile = Profile?.WithJobTitle(job.ID, null);
                            else
                                Profile = Profile?.WithJobTitle(job.ID, locId);
                            SetDirty();
                        };
                    }

                    _jobPriorities.Add((job.ID, selector));
                    _jobTitleButtons.Add(titleButton);
                    jobContainer.AddChild(selector);
                    jobContainer.AddChild(titleButton);
